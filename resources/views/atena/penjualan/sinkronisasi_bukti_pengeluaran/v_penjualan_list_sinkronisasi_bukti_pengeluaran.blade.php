...
@section('content')
              ...
              <table>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <a class="easyui-linkbutton" onclick="tampil_data()">Tampilkan SO</a>
                  </td>
                </tr>
                <tr>
  ...
  <div id="toolbar" style="padding: 5px">
    <a class="easyui-linkbutton" onclick="kalkulasi()">Kalkulasi</a>
  </div>
@endsection
  ...
  <script>
    ...
    function buat_table_detail() {
      ...
      $(dg).datagrid({
        ...
        onExpandRow: function(indexheader, rowheader) {
          ...
          ddv.datagrid({
            ...
            onCheck: function(index, row) {
              ...
              checkbox_header_trigger_enable = false;

              $('#table_data_detail').datagrid('checkRow', indexheader);

              checkbox_header_trigger_enable = true;
            },
            ...
            onUncheck: function(index, row) {
              ...
              var jumlah_tercentang = $(this).datagrid('getRows').filter(function(item) {
                return item.selected == 1;
              }).length;

              if (jumlah_tercentang == 0) {
              ...
              $('#table_data_detail').datagrid('unCheckRow', indexheader);
            },
        ...
        onCheck: function(index, row) {
          if (!checkbox_header_trigger_enable) {
            return false;
          }
        ...
        onCheckAll: function(rows) {
          ...
          for (var j = 0; j < rows.length; j++) {
            for (var i = 0; i < rows[j].detail.length; i++) {
              rows[j].detail[i].selected = 1;
            }
    ...
    function buat_table_detail_rekap() {
      ...
      $(dg).datagrid({
        ...
        rowStyler: function(index, row) {
          if (parseFloat(row.stok) < parseFloat(row.jml)) {
            return 'background-color: #f3c7c7';
          }
        }
      })
    }
    ...
    async function simpan() {
      ...
      var rows_rekap = $('#table_data_detail_rekap').datagrid('getRows');
      var data_detail = $('#table_data_detail').datagrid('getRows').filter(function(item) {
        return item.selected == 1;
      });

      if (!valid_kalkulasi) {
        $.messager.alert('Peringatan', 'Anda Belum Melakukan Proses Kalkulasi', 'warning');

        return false;
      }

      for (var i = 0; i < rows_rekap.length; i++) {
        if (parseFloat(rows_rekap[i].stok) < parseFloat(rows_rekap[i].jml)) {
          $.messager.alert('Peringatan', 'Terdapat Barang Dengan Stok Minus. Mohon Cek Pada Tab Rekap', 'error');
    ...
    async function kalkulasi() {
      ...
      var data_detail = data.filter(function(item) {
        return item.selected == 1;
      });
      ...
      try {
        ...
        if (res.success) {
          if (res.data.length == 0) {
            $.messager.alert('Peringatan', 'Belum Terdapat Data Barang Yang Dipilih', 'warning');

            return false;
          }

          valid_kalkulasi = true;
