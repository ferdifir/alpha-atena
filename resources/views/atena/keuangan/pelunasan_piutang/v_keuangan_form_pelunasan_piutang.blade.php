...
<div id="form_input" class="easyui-layout" fit="true">
	<div data-options="region:'center',border:false">
		...
		<input type="hidden" id="UUIDPELUNASAN" name="uuidpelunasan">
						...
						<table>
							<tr>
								...
								<td valign="bottom">
									<table border="0">
										<tr>
											<td id="label_form"><textarea name="catatan2" class="label_input" id="CATATAN2" multiline="true" prompt="Catatan" style="width:300px; height:95px" data-options="validType:'length[0, 500]'"></textarea></td>
										</tr>
									</table>
								</td>
							</tr>
					...
					<div id="tb-filter">
						<table id="fm-filter">
							<tr>
								...
								<td><input name="uuidcustomer" id="UUIDCUSTOMER" style="width:228px"></td>
							</tr>
							<tr hidden>
								<td id="label_form">Kota</td>
								<td><input name="kotacustomer" id="KOTACUSTOMER" style="width:228px" readonly></td>
							</tr>
							<tr>
								<td id="label_form" valign="top">Tanggal</td>
								<td id="label_form">
								...
								<td valign="bottom">
									<a id="btn_tampil"  class="easyui-linkbutton" data-options="iconCls:'icon-search', plain:false" onclick="tampil_data()">Tampilkan</a>
									<a id="btn_reset"  class="easyui-linkbutton" data-options="iconCls:'icon-reload', plain:false" onclick="reset_detail()">Reset Data</a>
								</td>
							</tr>
						</table>
					</div>
...
$(document).ready(async function(){
	...
	$('#NILAIKURS').numberbox({
		onChange: function(newVal, oldVal) {
			...
			if (row && row.uuidcurrency == '{{ session('UUIDCURRENCY') }}') {
...
async function ubah() {
	...
	const response = await fetchData(
		'{{ session('TOKEN') }}',
		link_api.loadDataHeaderPelunasanPiutang, {
		uuidpelunasan: '{{ $data }}'
		}
	);
	if(response.success) {
	...
	if (row) {
		...
		get_akses_user('<?= $kodemenu ?>', 'bearer {{ session('TOKEN') }}', async function(data) {
			...
			get_status_trans('{{ session("TOKEN") }}', "atena/keuangan/pelunasan-piutang", "uuidpelunasan", row.uuidpelunasan, async function(data) {
				...
				$('#UUIDCUSTOMER').combogrid('readonly', true);
...
async function simpan(jenis_simpan) {
	...
	if (cekbtnsimpan && isValid && (mode == 'tambah' || mode == 'ubah')) {
		...
		try {
			...
			let url = link_api.simpanPelunasanPiutang;
...
async function get_jurnal() {
	...
	if ($('#mode').val() == 'tambah') {
		...
		// Mengambil daftar customer dari table_data_detail yang dicentang
		for (var i = 0; i < rows.length; i++) {
			var customer_name = rows[i].namacustomer;
			var customer_kota = rows[i].kotacustomer || '';

			// Cek apakah customer sudah ada dalam daftar
			var exists = daftar_customer.filter(function(item) {
				return item.nama === customer_name;
			});

			if (exists.length === 0) {
				daftar_customer.push({
					nama: customer_name,
					kota: customer_kota
				});
			}
		...
		for (var i = 0; i < daftar_customer.length; i++) {
			referensi += daftar_customer[i].nama;

			if (daftar_customer[i].kota != null && daftar_customer[i].kota != '') {
				referensi += ` - (${daftar_customer[i].kota})`;
			}

			if (i != daftar_customer.length - 1) {
				referensi += ', ';
			}
		}
	...
	try {
		const response = await fetchData(
			'{{ session('TOKEN') }}',
			link_api.getJurnalLinkPelunasanPiutang, {
				data_detail  : rows,
...
async function load_data(uuidpelunasan, kodepelunasan) {
	try {
		const response = await fetchData(
			'{{ session('TOKEN') }}',
			link_api.loadDataPelunasanPiutang, {
		...
		if(response.success) {
			...
			$('#UUIDCUSTOMER').combogrid('setValues', {
				uuidcustomer: msg.referensi.uuid,
				nama: msg.referensi.nama
			});
...
async function tampil_data() {
	...
	if (uuidlokasi == '') {
		$.messager.alert('Warning', 'Lokasi Harus Dipilih', 'warning');
	...
	try {
		...
		if(response.success) {
			...
			if (msg.data_detail.length == 0) {
				$.messager.alert('Info', 'Tidak ada piutang', 'info');
			} else {
				var detail_trans = $('#table_data_detail').datagrid('getRows');

				for (var i = 0; i < msg.data_detail.length; i++) {
					var exists = detail_trans.filter(function (item) {
						return item.kodetrans == msg.data_detail[i].kodetrans;
					})
					...
					$('#table_data_detail').datagrid('appendRow', msg.data_detail[i]);
				}
			}
		...
		onChange: function(index, data) {
			...
			if (row) {
				...
				for (var i = 0; i < data_perkiraan.length; i++) {
					if (data_perkiraan[i].uuidperkiraan == row.id) {
						$.messager.show({
							title  : 'Warning',
...
function buat_table_detail() {
	...
	$(dg).datagrid({
		...
		columns      : [[
			...
			{field:'kodetrans',title:'No. Transaksi',width:110,},
			{field:'namacustomer',title:'Customer',width:180,},
			{field:'uuidcustomer',hidden:true},
			{field:'jenistransaksi',title:'Jenis Trans',width:80,},
...
function buat_table_detail_perkiraan() {
	...
	$("#table_data_perkiraan").datagrid({
		...
		columns    : [[
			...
			{field:'idcurrency',title:'Curr Kode', width:50, hidden:true,},
