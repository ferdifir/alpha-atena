...
@push('js')
  <script>
    ...
    $(document).ready(async function() {
      await get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(
        data) {
    ...
    async function before_add_table_pending(uuidbbm) {
      bukaLoader();
      try {
        let url = link_api.cekBisaBerlanjutBuktiPenerimaanBarang;
        ...
        if (response.success && response.data.bisa_berlanjut) {
          before_add(uuidbbm);
        } else {
    ...
    function before_add(uuidTransReferensi = "") {
      $('#mode').val('tambah');
      get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.tambah == 1) {
          parent.buka_submenu(null, 'Tambah Pembelian',
            '{{ route('atena.pembelian.pembelian.form', ['kode' => $kodemenu, 'mode' => 'tambah']) }}&data=' +
            uuidTransReferensi,
            'fa fa-plus')
        } else {
    ...
    async function before_print() {
      ...
      if (row) {
        get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', async function(data) {
          ...
          var checkTabAvailable = parent.check_tab_exist(row.kodebeli, 'fa fa-pencil');
          if (statusTrans == 'I') {
            var kode = row.kodebeli;
            if (checkTabAvailable) {
              $.messager.alert('Warning', 'Harap Tutup Tab Atas Transaksi ' +
                kode + ', Sebelum Dicetak ', 'warning');
            } else {
              try {
                let url = link_api.ubahStatusJadiSlipPembelian;
                const response = await fetch(url, {
                  method: 'POST',
                  headers: {
                    'Authorization': 'bearer {{ session('TOKEN') }}',
        ...
        if (statusTrans == "I" || statusTrans == "S") {
          $.messager.confirm('Confirm', 'Anda Yakin Membatalkan Transaksi Ini ?', async function(r) {
            if (r) {
              bukaLoader();
              try {
                let url = link_api.batalTransPembelian;
                const response = await fetch(url, {
                  method: 'POST',
                  headers: {
                    'Authorization': 'bearer {{ session('TOKEN') }}',
    ...
    async function batal_cetak() {
      ...
      if (row) {
        ...
        var checkTabAvailable = parent.check_tab_exist(row.kodebeli, 'fa fa-pencil');
        if (checkTabAvailable) {
          $.messager.alert('Warning', 'Harap Tutup Tab Atas Transaksi ' + row.kodebeli +
        ...
        if (statusTrans == "S") {
          $.messager.confirm('Confirm', 'Anda Yakin Batal Cetak Transaksi Ini ?', async function(r) {
            if (r) {
              bukaLoader();
              try {
                let url = link_api.ubahStatusJadiInputPembelian;
                const response = await fetch(url, {
                  method: 'POST',
                  headers: {
                    'Authorization': 'bearer {{ session('TOKEN') }}',
    ...
    function filter_data() {
      ...
      $('#table_data').datagrid('load', {
        ...
        noinvoicesupplier: $('#txt_noinvoicesupplier_filter').val(),
        kodebbm: $('#txt_kodebbm_filter').val(),
    ...
    function buat_table() {
      $("#table_data").datagrid({
        ...
        autoRowHeight: true,
        pageSize: 20,
        url: link_api.loadDataGridPembelian,
        pagination: true,
        clientPaging: false,
    ...
    function buat_table_pending() {
      $("#table_pending").datagrid({
        ...
        striped: true,
        rownumbers: true,
        url: link_api.loadDataGridPending,
        pagination: true,
        clientPaging: false,
        onLoadSuccess: function() {
          $('#table_pending').datagrid('unselectAll');
        },
        columns: [
          [{
              field: 'tgltrans',
        ...
        onDblClickRow: function(index, data) {
          before_add_table_pending(data.uuidbbm);
        }
      });
    }
