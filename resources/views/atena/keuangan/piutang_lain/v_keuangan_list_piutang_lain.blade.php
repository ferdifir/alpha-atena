...
function before_add() {
    get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.tambah == 1) {
            parent.buka_submenu(null, 'Tambah Piutang Lain',
                '{{ route('atena.keuangan.piutang_lain.form', ['kode' => $kodemenu, 'mode' => 'tambah', 'data' => '']) }}',
                'fa fa-plus')
        } else {
    ...
    get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.ubah == 1 || data.data.hakakses == 1) {
            ...
            parent.buka_submenu(null, row.kodetrans,
                '{{ route('atena.keuangan.piutang_lain.form', ['kode' => $kodemenu, 'mode' => 'ubah']) }}&data=' +
                row.kodetrans,
                'fa fa-pencil');
        } else {
            $.messager.alert('Warning', 'Anda Tidak Memiliki Hak Akses', 'warning');
...
async function batal_trans() {
    ...
    if (row && alasan != "") {
        ...
        var statusTrans = await getStatusTrans(link_api.getStatusPiutangLain,
        ...
        if (statusTrans == "I" || statusTrans == "S") {
            $.messager.confirm('Confirm', 'Anda Yakin Membatalkan Transaksi Ini ?', async function(r) {
                if (r) {
                    bukaLoader();
                    try {
                        let url = link_api.batalPiutangLain;
...
function buat_table() {
	$("#table_data").datagrid({
		...
		rownumbers  : true,
		pageSize    : 20,
		url         : link_api.loadDataGridPiutangLain,
		pagination  : true,
		clientPaging: false,
		frozenColumns: [
            [{
