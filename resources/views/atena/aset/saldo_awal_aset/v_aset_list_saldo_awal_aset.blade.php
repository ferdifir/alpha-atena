      ...
      <div class="easyui-layout" fit="true">
        <div data-options="region:'west',split:true,hideCollapsedContent:false,collapsed:false" title="Filter"
          ...
          <table border="0">
            ...
            <tr>
              <td id="label_form" align="center">No. Saldo Aset</td>
            </tr>
            <tr>
...
@push('js')
  ...
  <script>
    ...
    var edit_row = false;
    var idtrans = "";
    var counter = 0; //UNTUK TARGET FORM
    ...
    function before_add() {
      ...
      get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        data = data.data;
        if (data.tambah == 1) {
          parent.buka_submenu(null, 'Tambah Saldo Awal Aset',
            '{{ route('atena.aset.saldoawalaset.form', ['kode' => $kodemenu, 'mode' => 'tambah', 'data' => '']) }}',
            'fa fa-plus');
        } else {
    ...
    function batal_trans() {
      ...
      if (row) {
        $.messager.confirm(
          'Confirm',
          'Anda Yakin Akan Membatalkan Transaksi ' + row.kodesaldoaset + ' ?',
          async function(r) {
            if (r) {
              try {
                bukaLoader();
                const response = await fetch(
                  link_api.batalTransSaldoAwalAset, {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                      uuidsaldoaset: row.uuidsaldoaset,
                      kodesaldoaset: row.kodesaldoaset,
                      alasan: alasan
                    })
                  }
                );
    ...
    function batal_cetak() {
      if (row) {
        $.messager.confirm(
          'Confirm',
          'Anda Yakin Akan Batal Cetak Transaksi ' + row.kodesaldoaset + ' ?',
          async function(r) {
            if (r) {
              try {
                bukaLoader();
                const response = await fetch(
                  link_api.ubahStatusJadiInputSaldoAwalAset, {
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
                      'Authorization': 'Bearer {{ session('TOKEN') }}'
        ...
        get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', async function(data) {
          ...
          if (status == 'I') {
            try {
              bukaLoader();
              const response = await fetch(
                link_api.ubahStatusJadiSlipSaldoAwalAset, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer {{ session('TOKEN') }}'
              ...
              if (response.ok) {
                ...
                if (data.success) {
                  ...
                  const doc = await getCetakDocument(
                    link_api.cetakSaldoAwalAset + row.uuidsaldoaset,
                    '{{ session('TOKEN') }}'
                  );
                  if (doc) {
                    $("#area_cetak").html(doc);
    ...
    function buat_table() {
      $("#table_data").datagrid({
        ...
        clientPaging: false,
        pageSize: 20,
        url: link_api.loadDataGridSaldoAwalAset,
        ...
        onDblClickRow: function(index, data) {
          const kode = data.kodesaldoaset;
          parent.buka_submenu(null, kode,
            '{{ route('atena.aset.saldoawalaset.form', ['kode' => $kodemenu, 'mode' => 'ubah']) }}&data=' +
            data.uuidsaldoaset, 'fa fa-pencil');
        },
      });
    }
