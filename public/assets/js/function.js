...
function status_transaksi(statusTrans) {
	...
	if (statusTrans == "S") {
		linkTrans = "<img style='height:100px;' src='" + base_url + "assets/images/cetak_transparan.png'>";
	}
	if (statusTrans == "R") {
		linkTrans = "";
	}
	if (statusTrans == "P" || statusTrans == "T") {
		linkTrans = "<img style='height:100px;' src='" + base_url + "assets/images/posting_transparan.png'>";
	}
	if (statusTrans == "D") {
		linkTrans = "<img style='height:100px;' src='" + base_url + "assets/images/batal_transparan.png'>";
	}
	return linkTrans;
}
	...
	$.ajax({
		...
		success: function (msg) {
			if (msg.success) {
				...
				if (first_login) {
					// jika jumlah perusahaan yang bisa dimasuki oleh user hanya 1,
					// maka langsung dialihkan ke halaman daftar menu
					if (msg.perusahaan.length == 1) {
						window.location = base_url + "Home";
					}
					// jika jumlah perusahaan yang bisa dimasuki oleh user lebih dari 1,
					// maka dialihkan ke halaman daftar perusahaan
					else {
...
function get_data_user(kodemenu, callback) {
	$.ajax({
		dataType: "json",
		type    : 'POST',
		url     : base_url + "Master/Data/User/getUserAksesAuth",
		data    : "kodemenu=" + kodemenu + "&" + $("#form_login :input").serialize(),
		cache   : false,
		success : function (msg) {
			if (msg.success) {
				callback(msg.data);
...
function export_excel(file_name, data) {
	var str = "<form method='post' action='/export-excel' style='display:none' id='ReportTableData'>";
...
function get_kurs(tanggal, id) {
	...
	$.ajax({
		...
		dataType: "json",
		type: 'POST',
		url: base_url + "Master/Data/Currency/rate",
		data: {
			tanggal: tanggal,
			idcurrency: id
		},
...
function get_all_kurs(tanggal, callback) {
	$.ajax({
		dataType: "json",
		type: 'POST',
		url: base_url + "Master/Data/Currency/all_rate",
		data: {
			tanggal: tanggal
		},
		cache: false,
...
function cek_tanggal_jam_input(data, callback) {
    $.ajax({
        type: 'POST',
        dataType: 'json',
        ...
        success: function(response) {
            if (!response.success) {
                var errorMsg = 'Sudah Terdapat Perubahan Data Atas Transaksi Ini Yang Dilakukan Pada Tanggal ' + response.tgl + ' / ' + response.jam + '.<br>Transaksi Tidak Dapat Disimpan.';
...
function datagrid_row_color_styler(status) {
	if (status == 'S') {
		return 'background-color: ' + warna_status_s;
	}

	if (status == 'P') {
		return 'background-color: ' + warna_status_p;
	}

	if (status == 'D') {
		return 'background-color: ' + warna_status_d;
	}
}
...
function format_namabarang(val, row) {
	var nama       = typeof row.namabarang != 'undefined' && row.namabarang != null ? row.namabarang : '';
	var tipe       = typeof row.tipe != 'undefined' && row.tipe != null ? row.tipe : '';
	var jenisframe = typeof row.jenisframe != 'undefined' && row.jenisframe != null ? row.jenisframe : '';

	return nama + ' ' + tipe + ' ' + jenisframe;
}

function format_namabarang_combogrid(val, row) {
	var nama       = typeof row.nama != 'undefined' && row.nama != null ? row.nama : '';
	var tipe       = typeof row.tipe != 'undefined' && row.tipe != null ? row.tipe : '';
	var jenisframe = typeof row.jenisframe != 'undefined' && row.jenisframe != null ? row.jenisframe : '';
...
function cek_hakakses_simpan_cetak(kodemenu) {
    //TAMBAH CHECK AKSES CETAK DI WAKTU TAMBAH
