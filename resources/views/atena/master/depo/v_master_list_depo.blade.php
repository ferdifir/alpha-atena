  ...
  <div class="easyui-layout" fit="true">
    ...
    <div data-options="region: 'center'">
      <div class="easyui-layout" style="width:100%;height:100%" fit="true">
        <div data-options="region:'center',">
          <table id="table_data" idField="uuiddepo"></table>
        </div>
      </div>
    </div>
  </div>
      ...
      $('#table_data').datagrid({
        ...
        columns: [
          [{
            ...
            {
              field: 'toleransijarak',
              title: 'Toleransi Jarak',
              width: 100,
              sortable: true,
            },
            {
              field: 'toleransijarakkirim',
              title: 'Toleransi Jarak Kirim',
              width: 125,
              sortable: true,
            },
            {
    ...
    function before_add() {
      get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.tambah == 1) {
          parent.buka_submenu(null, 'Tambah Depo',
            '{{ route('atena.master.depo.form', ['kode' => $kodemenu, 'mode' => 'tambah', 'data' => '']) }}',
            'fa fa-plus')
        } else {
    ...
    function before_edit() {
      get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.ubah == 1 || data.data.hakakses == 1) {
          ...
          parent.buka_submenu(null, row.namadepo,
            '{{ route('atena.master.depo.form', ['kode' => $kodemenu, 'mode' => 'ubah']) }}&data=' +
            row.uuiddepo,
            'fa fa-pencil');
        } else {
          $.messager.alert('Warning', 'Anda Tidak Memiliki Hak Akses', 'warning');
