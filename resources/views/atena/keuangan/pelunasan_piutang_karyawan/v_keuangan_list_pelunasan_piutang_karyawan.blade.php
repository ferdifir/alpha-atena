        ...
        <div class="easyui-layout" fit="true">
            <div data-options="region:'west',split:true,hideCollapsedContent:false,collapsed:false" title="Filter" style="width:150px;" align="center">
                <table border="0">
                    ...
                    <tr>
                        <td id="label_form" align="center">Karyawan</td>
                    </tr>
                    <tr>
                        <td align="center"><input id="txt_nama_karyawan_filter" name="txt_nama_karyawan_filter" style="width:100px" class="label_input" /></td>
                    </tr>
                    <tr>
                        <td id="label_form"><br></td>
                    </tr>
...
function before_add() {
    ...
    get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.tambah == 1) {
            parent.buka_submenu(null, 'Tambah Pelunasan Piutang Karyawan',
                '{{ route('atena.keuangan.pelunasan_piutang_karyawan.form', ['kode' => $kodemenu, 'mode' => 'tambah', 'data' => '']) }}',
                'fa fa-plus')
        } else {
    ...
    get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
        if (data.data.ubah == 1 || data.data.hakakses == 1) {
            ...
            parent.buka_submenu(null, row.kodepelunasan,
                '{{ route('atena.keuangan.pelunasan_piutang_karyawan.form', ['kode' => $kodemenu, 'mode' => 'ubah']) }}&data=' +
                row.uuidpelunasan,
                'fa fa-pencil');
        } else {
            $.messager.alert('Warning', 'Anda Tidak Memiliki Hak Akses', 'warning');
...
async function before_print() {
    ...
    if (row) {
        get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', async function(data) {
            ...
            var statusTrans = await getStatusTrans(link_api.getStatusPelunasanPiutangKaryawan,
            ...
            if (statusTrans == 'I') {
                ...
                if (checkTabAvailable) {
                    ...
                    try {
                        let url = link_api.ubahStatusjadiSlipPelunasanPiutangKaryawan;
        ...
        if (statusTrans == "I" || statusTrans == "S") {
            $.messager.confirm('Confirm', 'Anda Yakin Membatalkan Transaksi Ini ?', async function(r) {
                if (r) {
                    bukaLoader();
                    try {
                        let url = link_api.batalPelunasanPiutangKaryawan;
                        const response = await fetch(url, {
                            method: 'POST',
                            headers: {
                                'Authorization': 'bearer {{ session('TOKEN') }}',
        ...
        if (statusTrans == "S") {
            $.messager.confirm('Confirm', 'Anda Yakin Batal Cetak Transaksi Ini ?', async function(r) {
                if (r) {
                    bukaLoader();
                    try {
                        let url = link_api.ubahStatusjadiInputPelunasanPiutangKaryawan;
                        const response = await fetch(url, {
                            method: 'POST',
                            headers: {
                                'Authorization': 'bearer {{ session('TOKEN') }}',
...
async function cetak(uuidtrans) {
    ...
    if (row) {
        try {
            let url = link_api.cetakPelunasanPiutangKaryawan + uuidtrans;
            const response = await fetch(url, {
                method: 'POST',
                headers: {
                    'Authorization': 'bearer {{ session('TOKEN') }}',
...
function filter_data() {
    ...
    $('#table_data').datagrid('load', {
        ...
        referensi: $('#txt_nama_karyawan_filter').val(),
...
function buat_table() {
    ...
    $("#table_data").datagrid({
        ...
        rownumbers  : true,
        pageSize    : 20,
        url         : link_api.loadDataGridPelunasanPiutangKaryawan,
        pagination  : true,
        clientPaging: false,
