                    ...
                    <div class="easyui-layout" fit="true">
                        <div data-options="region:'west',split:true,hideCollapsedContent:false,collapsed:false"
                            ...
                            <table border="0">
                                ...
                                <tr>
                                    <td id="label_form" align="center">No. Pemakaian</td>
                                </tr>
                                <tr>
    ...
    <script>
        ...
        function before_edit() {
            ...
            get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
                if (data.data.ubah == 1 || data.data.hakakses == 1) {
                    ...
                    parent.buka_submenu(null, row.kodepemakaianbahan,
                        '{{ route('atena.inventori.pemakaian_bahan.form', ['kode' => $kodemenu, 'mode' => 'ubah']) }}&data=' +
                        row.uuidpemakaianbahan,
                        'fa fa-pencil');
                } else {
                    $.messager.alert('Warning', 'Anda Tidak Memiliki Hak Akses', 'warning');
                }
        ...
        async function batal_trans() {
            ...
            if (row && alasan != "") {
                ...
                var statusTrans = await getStatusTrans(link_api.getStatusTransaksiPemakaianBahan,
                ...
                if (statusTrans == "I" || statusTrans == "S") {
                    $.messager.confirm('Confirm', 'Anda Yakin Menghapus Data Ini ?', async function(r) {
                        if (r) {
                            bukaLoader();
                            try {
                                let url = link_api.batalTransaksiPemakaianBahan;
                ...
                get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', async function(data) {
                    ...
                    if (statusTrans == 'I') {
                        ...
                        if (checkTabAvailable) {
                            ...
                            try {
                                let url = link_api.ubahStatusJadiSlipPemakaianBahan;
                                const response = await fetch(url, {
                                    method: 'POST',
                                    headers: {
                                        'Authorization': 'bearer {{ session('TOKEN') }}',
        ...
        function buat_table() {
            $("#table_data").datagrid({
                ...
                rownumbers: true,
                pageSize: 20,
                url: link_api.loadDataGridPemakaianBahan,
                pagination: true,
                clientPaging: false,
                ...
                frozenColumns: [
                    [{
                        ...
                        {
                            field: 'uuidpemakaianbahan',
                            hidden: true
                        },
                        {
                            field: 'kodepemakaianbahan',
                            title: 'No. Pemakaian Bahan',
                            width: 140,
                            sortable: true,
                            align: 'center'
                        },
