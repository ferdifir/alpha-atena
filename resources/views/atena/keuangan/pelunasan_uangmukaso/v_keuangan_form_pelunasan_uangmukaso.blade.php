                    ...
                    <div id="tb-filter">
                        <table id="fm-filter">
                            ...
                            <tr>
                                ...
                                <td valign="bottom">
                                    <a id="btn_tampil" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="tampil_data()">Tampilkan</a>
                                </td>
                            </tr>
                        </table>
                    </div>
...
$(document).ready(function() {
	...
	browse_data_referensi('#UUIDCUSTOMER');
...
async function cetak(uuid) {
    const doc = await getCetakDocument(
		'{{ session('TOKEN') }}',
		link_api.cetakPelunasanUangMukaSO + uuid
	);
	if (doc == null) {
...
async function ubah() {
	...
	const response = await fetchData(
		'{{ session('TOKEN') }}',
		link_api.loadDataHeaderPelunasanUangMukaSO, {
		uuidpelunasan: '{{ $data }}'
		}
	);
	if(response.success) {
	...
	if (row) {
		...
		get_akses_user('<?= $kodemenu ?>', 'bearer {{ session('TOKEN') }}', async function(data) {
			...
			get_status_trans('{{ session("TOKEN") }}', "atena/keuangan/pelunasan-uang-muka-pesanan-penjualan", "uuidpelunasan", row.uuidpelunasan, async function(data) {
...
async function simpan(jenis_simpan) {
	...
	if (cekbtnsimpan && isValid && (mode == 'tambah' || mode == 'ubah')) {
		...
		try {
			...
			let url = link_api.simpanPelunasanUangMukaSO;
...
async function get_jurnal() {
	...
	if ($('#mode').val() == 'tambah') {
		var referensi = '';
		var daftar_customer = $('#UUIDCUSTOMER').combogrid('grid').datagrid('getSelections');
...
async function load_data(idtrans, kodetrans) {
	try {
		const response = await fetchData(
			'{{ session('TOKEN') }}',
			link_api.loadDataPelunasanUangMukaSO, {
		...
		if(response.success) {
			...
			$('#UUIDCUSTOMER').combogrid('setValues', msg.referensi);
	...
	try {
		...
		if(response.success) {
			...
			if (msg.data_detail.length == 0) {
				$.messager.alert('Warning', 'Tidak Ada Uang Muka', 'warning');
			}
		...
		onEndEdit: function(index, row, changes) {
			...
			switch (cell.field) {
				case 'kodeperkiraan':
					...
					var uuidperkiraan  = data ? data.uuidperkiraan : '';
