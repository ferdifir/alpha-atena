...
<body class="easyui-layout">
                                            ...
                                            @foreach ($menuLv2['children'] as $menuLv3)
                                                ...
                                                <a href="#" id="linkLaporan" class="Links"
                                                    ...
                                                    <div class="laporanSubItems">
                                                        ...
                                                        <i class="fa fa-caret-right iconColor "
                                                            style="margin-right:10px; font-size:10px;"></i>
                                                        <i class="{{ $menuLv2['icon'] }} iconColor "
    ...
    <div id="header" data-options="region:'north',border:false" style="padding:0px; height:50px;">
        ...
        <table width="100%" border="0"
            ...
            <tr class="logoheader">
                <td class="font-header-form" align="left" valign="center"
                    style="padding-left: 28px; width:120px;">
                    <img style="width: 100px;display: block" src="{{ asset('assets/images/logo_putih.png') }}">
                </td>
                <td align="left">
                    <img id="billing-image" style="height:50px;" src="{{ asset('assets/images/freetrial.png') }}"
                        hidden>
                </td>
                    ...
                    <table>
                        <tr>
                            <td class="font-header-form" style="font-size: 25px;padding-right: 10px;">
                                @if (session()->has('KODELOKASI'))
                                    {{ session('KODELOKASI') }}
                                @endif
                            </td>
                            <td class="label_form" hidden>
                                ...
                                <a href="javascript:void(0)" class="easyui-menubutton profile_btn"
                                    ...
                                    <table>
                                        <tr>
                                            <td>
                                                @php
                                                    $gambaruser = session('DATAUSER')['gambar'];
                                                @endphp
                                                <img src="{{ $gambaruser }}"
                                            ...
                                            <td>
                                                <div id="profile_menu" style="width:200px; border-radius:5px;">
                                                    <div onclick="javascript:edit_profile()"
                                                        data-options="iconCls:'icon-profile'">Profile</div>
                                                    @if (session()->has('LISTLOKASI') && session('LISTLOKASI')->count() > 1)
                                                        <div onclick="window.location.replace('{{ route('homepage.lokasi') }}')"
                                                            data-options="iconCls:'icon-logout'">Kembali ke Pilih
                                                            Lokasi</div>
                                                    @endif
                                                    @if (session()->has('LIST_PERUSAHAAN'))
                                                        <div onclick="window.location.replace('{{ route('homepage.perusahaan') }}')"
                                                            data-options="iconCls:'icon-logout'">Kembali ke Pilih
                                                            Perusahaan</div>
                                                    @endif
                                                    <div onclick="javascript:logout()"
                                                        data-options="iconCls:'icon-logout'">Keluar dari Aplikasi</div>
                                                </div>
                                            </td>
                                        </tr>
                                    </table>
    ...
    @if (!is_null(request()->get('kode')))
        ...
        <!-- Region west start -->
        <div data-options="region: 'west',split: true,hideCollapsedContent: false,collapsed: true,iconCls: 'fa fa-align-justify fa-fw'"
    ...
    <!-- Region center start -->
    <div data-options="region:'center'" id="v_modul">
        ...
        {{-- <form id="form_input_user" enctype="multipart/form-data" method="post" hidden>
            ...
            <table style="padding:2px" border="0">
                ...
                <tr>
                    <td align="right" id="label_form">Kata sandi saat ini</td>
                    <td><input name="old_pass_profile" id="OLD_PASS_PROFILE" type="password" class="label_input"
                            data-options="required:true,fontTransform:'normal'" validType='length[0,50]'
                            style="width:150px" /> </td>
                </tr>
                <tr>
                    <td align="right" id="label_form">Kata sandi baru</td>
                ...
                <tr>
                    <td align="right" id="label_form">Konfirmasi kata sandi baru</td>
    ...
    <script>
        ...
        function edit_profile() {
            ...
            $('#USERNAME_PROFILE').textbox('setValue', "");
            $('#EMAIL_PROFILE').textbox('setValue', "");
            $('#NOHP_PROFILE').textbox('setValue', "");
            $('#GAMBARUSER_PROFILE').val("");
            $('#IDUSER_PROFILE').val("");

            if ($('#GAMBARUSER_PROFILE').val() == "") {
                ...
                downloadingImage.src = "storage/foto_user/" + "";
            }
        }
