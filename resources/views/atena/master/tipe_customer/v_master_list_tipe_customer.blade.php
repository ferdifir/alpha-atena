            ...
            <div id="tab_transaksi" class="easyui-tabs" style="width:100%;height:100%;">
                <div title="Grid" id="Grid">
                    <div class="easyui-layout" style="width:100%;height:100%" fit="true">
                        <div data-options="region:'center',">
                            <table id="table_data" idField="uuidtipecustomer"></table>
                        </div>
                    </div>
                </diV>
            </div>
    ...
    <script>
        ...
        function before_add() {
            ...
            get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
                if (data.data.tambah == 1) {
                    parent.buka_submenu(null, 'Tambah Tipe Customer',
                        '{{ route('atena.master.tipe_customer.form', ['kode' => $kodemenu, 'mode' => 'tambah', 'data' => '']) }}',
                        'fa fa-plus')
                } else {
        ...
        function before_edit() {
            ...
            get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', function(data) {
                if (data.data.ubah == 1 || data.data.hakakses == 1) {
                    ...
                    parent.buka_submenu(null, row.namatipecustomer,
                        '{{ route('atena.master.tipe_customer.form', ['kode' => $kodemenu, 'mode' => 'ubah']) }}&data=' +
                        row.uuidtipecustomer,
                        'fa fa-pencil');
                } else {
                    $.messager.alert('Warning', 'Anda Tidak Memiliki Hak Akses', 'warning');
                }
        ...
        function buat_table() {
            $('#table_data').datagrid({
                ...
                pagination: true,
                pageSize: 20,
                url: link_api.loadDataGridTipeCustomer,
                clientPaging: false,
