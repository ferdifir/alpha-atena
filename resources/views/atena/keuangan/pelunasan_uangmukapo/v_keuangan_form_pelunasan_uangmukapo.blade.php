...
$(document).ready(async function() {
	...
	browse_data_referensi('#UUIDSUPPLIER');
...
async function cetak(uuid) {
    const doc = await getCetakDocument(
		'{{ session('TOKEN') }}',
		link_api.cetakPelunasanUangMukaPO + uuid
	);
	if (doc == null) {
	...
	if (row) {
		get_akses_user('<?= $kodemenu ?>', 'bearer {{ session('TOKEN') }}', async function(data) {
			...
			get_status_trans('{{ session("TOKEN") }}', "atena/keuangan/pelunasan-uang-muka-pesanan-pembelian", "uuuuidpelunasan", row.uuuuidpelunasan, async function(data) {
				...
				if (row.uuidmemo != '') {
					$('#UUIDKAS').combogrid('setValue', row.kodememo)
				} else {
					$('#UUIDKAS').combogrid('setValue', row.kodekas)
				}
...
async function simpan(jenis_simpan) {
	...
	var mode = $("#mode").val();
	var dataheader = $("#form_input :input").serialize() + "&NAMASUPPLIER=" + $('#UUIDSUPPLIER').combogrid('getText');
	...
	if (cekbtnsimpan && isValid && (mode == 'tambah' || mode == 'ubah')) {
		...
		try {
			...
			let url = link_api.simpanPelunasanUangMukaPO;
...
async function get_jurnal() {
	...
	var namasupplier = $('#UUIDSUPPLIER').combogrid('getText');

	$('#CATATAN').textbox('setValue', 'PELUNASAN UANG MUKA KE ' + namasupplier);
...
async function load_data(uuidpelunasan, kodepelunasan) {
	try {
		const response = await fetchData(
			'{{ session('TOKEN') }}',
			link_api.loadDataPelunasanUangMukaPO, {
	...
	$(id).combogrid({
		...
		onChange: function(index, data) {
			...
			if (row) {
				...
				$('#UUIDCURRENCY').combogrid('setValue', row.idcurrency);
