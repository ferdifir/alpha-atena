...
function before_print() {
    ...
    if (row) {
        get_akses_user('{{ $kodemenu }}', 'bearer {{ session('TOKEN') }}', async function(data) {
            ...
            var checkTabAvailable = parent.check_tab_exist(row.kodesaldoperkiraan, 'fa fa-pencil');
            if (statusTrans == 'I') {
                var kode = row.kodesaldoperkiraan;
                if (checkTabAvailable) {
                    $.messager.alert('Warning', 'Harap Tutup Tab Atas Transaksi ' +
                        kode + ', Sebelum Dicetak ', 'warning');
                } else {
                    try {
                        let url = link_api.ubahStatusjadiSlipSaldoAwalPerkiraan;
                        const response = await fetch(url, {
                            ...
                            body: JSON.stringify({
                                uuidsaldoperkiraan: row.uuidsaldoperkiraan,
                                kodesaldoperkiraan: row.kodesaldoperkiraan,
                            }),
                        }).then(response => {
                            if (!response.ok) {
                                throw new Error(
...
async function batal_trans() {
    ...
    if (row && alasan != "") {
        ...
        var checkTabAvailable = parent.check_tab_exist(row.kodesaldoperkiraan, 'fa fa-pencil');
        if (checkTabAvailable) {
            $.messager.alert('Warning', 'Harap Tutup Tab Atas Transaksi ' + row.kodesaldoperkiraan +
        ...
        if (statusTrans == "I" || statusTrans == "S") {
            $.messager.confirm('Confirm', 'Anda Yakin Membatalkan Transaksi Ini ?', async function(r) {
                if (r) {
                    bukaLoader();
                    try {
                        let url = link_api.batalSaldoAwalPerkiraan;
                        const response = await fetch(url, {
                            method: 'POST',
                            headers: {
                                'Authorization': 'bearer {{ session('TOKEN') }}',
        ...
        if (statusTrans == "S") {
            $.messager.confirm('Confirm', 'Anda Yakin Batal Cetak Transaksi Ini ?', async function(r) {
                if (r) {
                    bukaLoader();
                    try {
                        let url = link_api.ubahStatusjadiInputSaldoAwalPerkiraan;
                        const response = await fetch(url, {
                            method: 'POST',
                            headers: {
                                'Authorization': 'bearer {{ session('TOKEN') }}',
...
function buat_table() {
    $("#table_data").datagrid({
        ...
        rownumbers: true,
        pageSize: 20,
        url: link_api.loadDataGridSaldoAwalPerkiraan,
        pagination: true,
        clientPaging: false,
