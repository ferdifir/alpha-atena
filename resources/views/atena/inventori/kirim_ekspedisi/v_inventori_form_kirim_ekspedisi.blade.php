...
@push('js')
  ...
  <script>
    ...
    async function load_data(idtrans) {
      try {
        const res = await fetchData(link_api.loadDataInventoryKirimEkspedisi, {
          uuidkirim: idtrans
        });
        if (res.success) {
    ...
    async function load_data_detail(idcustomer) {
      try {
        const res = await fetchData(link_api.loadDataDetailKirimBarangKeluar, {
