...
async function batal_trans() {
    ...
    if (row && alasan != "") {
        ...
        var statusTrans = await getStatusTrans(link_api.getStatusSaldoAwalPiutang,
        ...
        if (statusTrans == "I" || statusTrans == "S") {
            $.messager.confirm('Confirm', 'Anda Yakin Membatalkan Transaksi Ini ?', async function(r) {
                if (r) {
                    bukaLoader();
                    try {
                        let url = link_api.batalSaldoAwalPiutang;
...
function buat_table() {
	$("#table_data").datagrid({
		...
		rownumbers  : true,
		pageSize    : 20,
		url         : link_api.loadDataGridSaldoAwalPiutang,
		pagination  : true,
		clientPaging: false,
		frozenColumns:[[
