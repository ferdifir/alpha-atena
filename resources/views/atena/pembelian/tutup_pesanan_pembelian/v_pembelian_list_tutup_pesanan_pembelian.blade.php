  ...
  <script>
    ...
    function tutup() {
      ...
      if (isValid && mode == 'tutup') {
        ...
        fetchData(
          '{{ session('TOKEN') }}',
          link_api.tutupTransaksiPesananPembelian,
          payload
        ).then(res => {
          if (res.success) {
    ...
    function tutup_detail() {
      ...
      if (isValid && mode == 'tutup') {
        ...
        fetchData(
          '{{ session('TOKEN') }}',
          link_api.tutupTransaksiBarangPesananPembelian,
          payload
        ).then(res => {
          if (res.success) {
            $.messager.alert('Info', 'Transaksi Sukses Ditutup', 'info');
    ...
    function browse_filter_lokasi(id, table) {
      $(id).combogrid({
        ...
        onChange: function(newVal, oldVal) {
          ...
          if (row) {
            url = link_api.browseFilterPesananPembelian;
    ...
    function buat_table() {
      $("#table_data").datagrid({
        ...
        selectOnCheck: false,
        RowAdd: false,
        url: link_api.loadDataGridTutupPesananPembelian,
    ...
    function load_data(uncheck = 0) {
      ...
      if (row) {
        ...
        fetchData(
          '{{ session('TOKEN') }}',
          link_api.loadDataRekapPesananPembelian,
          payload
        ).then(res => {
          if (res.success) {
